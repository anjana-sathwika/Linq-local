...
export default function Features() {
  ...
  return (
    <section className="bg-white text-center py-16 sm:py-20 md:py-24 px-4 sm:px-6 relative overflow-hidden">
      {/* decorative accents (behind content) — light blue/soft */}
      <div aria-hidden className="pointer-events-none absolute inset-0">
        <div className="absolute -left-12 sm:-left-24 -top-10 sm:-top-20 w-48 sm:w-72 h-48 sm:h-72 rounded-full bg-[#5FA9FF]/8 blur-3xl" />
        <div className="absolute -right-12 sm:-right-24 -top-5 sm:-top-10 w-60 sm:w-80 h-32 sm:h-40 rounded-3xl bg-[#5FA9FF]/6 opacity-20 transform rotate-12" />
      </div>

      <div className="max-w-6xl mx-auto relative z-10">
        <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-3 sm:mb-4 text-[#2F5EEA] px-4">
          Why Ride with LinQ?
        </h2>
        <p className="text-gray-600 text-sm sm:text-base md:text-lg mb-8 sm:mb-10 max-w-2xl mx-auto px-4">
          LinQ combines smart matching, transparent costs and a community-first approach to make commuting easier
          and more sustainable. Here are the core benefits you get as a member.
        </p>
        ...
        <div className="lg:hidden grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 max-w-4xl mx-auto">
          {features.map((f, i) => (
            <div 
              key={i} 
              className="p-6 sm:p-8 bg-white rounded-2xl sm:rounded-3xl border border-gray-200 hover:border-[#2F5EEA] transition-all hover:shadow-lg hover:shadow-[#5FA9FF]/20"
            >
              <div className="flex items-center justify-between mb-3 sm:mb-4">
                <h3 className="text-base sm:text-lg md:text-xl font-semibold text-gray-900">{f.title}</h3>
                <span className="inline-block w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-[#2F5EEA]" />
              </div>
              <p className="text-gray-600 text-xs sm:text-sm md:text-base text-left leading-relaxed">{f.desc}</p>
            </div>
          ))}
        </div>
              ...
              return features.map((f, i) => {
                ...
                return (
                  <div
                    ...
                    style={{ zIndex, transform, transition: 'transform 620ms cubic-bezier(.18,.9,.2,1), box-shadow 420ms ease' }}
                  >
                    <div className={`p-8 w-80 bg-white rounded-3xl border ${isActive ? 'border-[#2F5EEA] shadow-[0_40px_120px_rgba(47,94,234,0.2)]' : 'border-gray-200 shadow-lg'} `} style={{ transformOrigin: "center", transition: 'box-shadow 420ms ease, transform 620ms cubic-bezier(.18,.9,.2,1)' }}>
                      <div className="flex items-center justify-between mb-4">
                        <h3 className="text-lg md:text-xl font-semibold text-gray-900">{f.title}</h3>
                        <span className="inline-block w-3 h-3 rounded-full bg-[#2F5EEA] shadow-md" />
                      </div>
